...
public class LoganSquareCursorFieldConverterTest {
    ...
    public void testParseField() throws Exception {
        ...
        cursor.addRow(new String[]{
                LoganSquare.serialize(jsonObject),
                LoganSquare.serialize(Arrays.asList(jsonArray), Model.class),
                LoganSquare.serialize(jsonList, Model.class),
                LoganSquare.serialize(jsonMap, Model.class)
        });
        cursor.moveToFirst();
        ...
        assertEquals(jsonList, converter.parseField(cursor, 2, TypeUtils.parameterize(List.class, Model.class)));
        assertEquals(jsonMap, converter.parseField(cursor, 3, TypeUtils.parameterize(Map.class, String.class, Model.class)));
    }
    ...
    public void testWriteField() throws Exception {
        ...
        assertEquals(LoganSquare.serialize(jsonObject), contentValues.getAsString("json_object"));
        assertEquals(LoganSquare.serialize(Arrays.asList(jsonArray), Model.class), contentValues.getAsString("json_array"));
        assertEquals(LoganSquare.serialize(jsonList, Model.class), contentValues.getAsString("json_list"));
        assertEquals(LoganSquare.serialize(jsonMap, Model.class), contentValues.getAsString("json_map"));
    }
